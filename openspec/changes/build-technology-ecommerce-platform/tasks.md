...
- [x] 8.3 Implementar creación de factura manual para `ADMIN` y `BILLING`; verificar origen `MANUAL`, cliente y líneas válidas, ausencia de orden y cero movimientos de inventario.
- [x] 8.4 Implementar endpoints paginados de facturas con búsqueda, filtros, detalle y actualización de estado; verificar permisos administrativos y consulta exclusiva del cliente propietario.
- [x] 8.5 Crear las pantallas de facturas del back office, incluyendo conversión desde orden y formulario manual con React Hook Form y Zod; verificar flujos de Admin y Billing y acciones prohibidas para Customer.
- [x] 8.6 Mostrar facturas propias dentro del área de cliente cuando estén disponibles; verificar que nunca se expongan facturas ajenas.
...
- [ ] 21.1 Añadir migraciones para `Product.isFeatured`, `Product.featuredAt`, `Category.showOnLanding` y `Category.landingOrder`, con índices y restricciones de posición y máximo de tres categorías; verificar valores por defecto, unicidad de orden y migración desde una base existente.
- [ ] 21.2 Extender las mutaciones administrativas de productos y categorías para activar o retirar destaques y ordenar hasta tres categorías importantes; verificar permisos `ADMIN`, actualización de `featuredAt`, rechazo de una cuarta categoría, auditoría y conservación de asociaciones.
- [ ] 21.3 Implementar `GET /api/v1/catalog/landing` con destacados, recientes deduplicados y categorías ordenadas, y publicar su contrato OpenAPI, cliente y esquemas Zod; verificar límites 3/9/3, campos públicos, productos activos y una única respuesta coherente.
- [ ] 21.4 Integrar controles de destacar o retirar destaque en el listado y formulario administrativo de productos; verificar estado visible, mensaje flash, actualización de fecha solo al destacar e invalidación del resumen de landing.
- [ ] 21.5 Integrar selección, orden y retirada de dos o tres categorías importantes en el backoffice; verificar drag, controles alternativos de teclado, límite máximo, advertencia de configuración incompleta y persistencia de posiciones.
- [ ] 21.6 Ampliar el seed para marcar tres productos destacados y tres categorías importantes con orden determinista, conservando nueve productos activos adicionales; verificar idempotencia y una composición completa y reproducible de la landing.
- [ ] 21.7 Implementar en la landing las secciones ordenadas de tres destacados, nueve recientes sin repetir destacados y hasta tres categorías con tres productos cada una; verificar estados completos, parciales y vacíos, repetición permitida en categorías, responsive y los temas claro y oscuro.
- [ ] 21.8 Añadir pruebas de integración, contrato, autorización, componentes y end-to-end para destaques y categorías importantes, y actualizar README y AGENTS.md; verificar rechazo de `CUSTOMER`, omisión de inactivos, orden, límites, deduplicación, documentación alineada y validación OpenSpec estricta.
