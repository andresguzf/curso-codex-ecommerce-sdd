...
export class InventoryStockRepository {
  ...
  async deduct(
    ...
    return this.database.client.transaction((transaction) =>
      this.apply(transaction, items, reference, "SALE"),
    );
  }
  ...
  deductInTransaction(
    ...
    return this.apply(transaction, items, reference, "SALE");
  }
  ...
  private async apply(
        ...
        items
          .filter(
            (item) =>
              !lockedBalances.some(
                ({ item: lockedItem }) =>
                  lockedItem.productId === item.productId,
              ),
          )
          .map((item) => ({
            availableQuantity: 0,
    ...
    for (const current of lockedBalances) {
      ...
      if (!balance) {
        throw new Error("PostgreSQL did not return the inventory balance");
      }
