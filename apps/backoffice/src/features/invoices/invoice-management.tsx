...
import { ConfirmationDialog, DataTable, ErrorState, LoadingState, type DataTableColumn } from "@technology-ecommerce/ui";
...
export function InvoicesManagement() {
  ...
  const columns: readonly DataTableColumn<InvoiceSummary>[] = [
    ...
    { id: "actions", header: "Acciones", cell: (invoice) => <div className="flex min-w-56 flex-wrap gap-2"><Link className="rounded-md border border-blue-300 px-3 py-2 font-semibold text-blue-800 hover:bg-blue-50" href={`/invoices/${invoice.id}`}>Ver detalle</Link>{nextStatus(invoice.status) && nextStatus(invoice.status) !== "VOID" ? <button className="rounded-md border border-emerald-300 px-3 py-2 font-semibold text-emerald-800 hover:bg-emerald-50" disabled={statusMutation.isPending} onClick={() => statusMutation.mutate({ id: invoice.id, status: nextStatus(invoice.status)! })} type="button">{invoice.status === "DRAFT" ? "Emitir" : "Marcar pagada"}</button> : null}{nextStatus(invoice.status) === "VOID" ? <button className="rounded-md border border-red-300 px-3 py-2 font-semibold text-red-800 hover:bg-red-50" onClick={() => setVoidTarget(invoice)} type="button">Anular</button> : null}</div> },
  ];

  return <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-950 sm:px-8"><div className="mx-auto max-w-[90rem]"><header className="flex flex-wrap items-end justify-between gap-5 border-b border-slate-300 pb-6"><div><Link className="text-sm font-bold text-blue-800 hover:underline" href="/">← Panel principal</Link><p className="mb-0 mt-5 text-xs font-bold uppercase tracking-[.18em] text-blue-800">Operaciones · {session.user.role === "ADMIN" ? "Administración" : "Facturación"}</p><h1 className="mb-0 mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Facturas</h1><p className="mb-0 mt-2 max-w-2xl text-slate-600">Consulta, emite y administra facturas independientes de las órdenes.</p></div><div className="flex flex-wrap gap-3"><button className="min-h-11 rounded-lg border border-blue-300 px-4 py-2 font-bold text-blue-800 hover:bg-blue-50" onClick={() => { setManualOpen((open) => !open); setNotice(undefined); }} type="button">{manualOpen ? "Cerrar formulario" : "Nueva factura manual"}</button><form className="flex gap-2" onSubmit={submitOrder}><label className="sr-only" htmlFor="invoice-order-id">ID de orden</label><input className="min-h-11 w-64 rounded-lg border border-slate-300 px-3 font-mono text-xs" id="invoice-order-id" onChange={(event) => setOrderId(event.target.value)} placeholder="UUID de orden a facturar" value={orderId} /><button className="min-h-11 rounded-lg bg-[#15345b] px-4 py-2 font-bold text-white disabled:opacity-60" disabled={orderMutation.isPending} type="submit">{orderMutation.isPending ? "Facturando…" : "Facturar orden"}</button></form></div></header><div aria-atomic="true" aria-live="polite" className={`min-h-12 py-3 text-sm font-semibold ${notice?.kind === "success" ? "text-emerald-800" : "text-red-800"}`}>{notice?.message}</div>
