...
import { ConfirmationDialog, ErrorState, IconButton, LoadingState } from "@technology-ecommerce/ui";
import Link from "next/link";
import { useSessionStore } from "../auth/session";
import { downloadOrderPdf } from "./order-api";
...
export function AdministrativeOrderDetail({ orderId }: Readonly<{ orderId: string }>) {
  ...
  const mutations = useOrderMutations(
    (message) => { setCancelOpen(false); setCompleteOpen(false); setInvoiceOpen(false); setNotice({ kind: "success", message }); },
  ...
  const canInvoice = canManageOrders && order.status === "PROCESSING";
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-950 sm:px-8"><div className="mx-auto max-w-6xl">
    ...
    <header className="flex flex-wrap items-start justify-between gap-5 border-b border-slate-200 p-6"><div><Link className="text-sm font-bold text-blue-700 underline" href="/orders">← Volver al listado</Link><p className="mb-0 mt-5 break-all font-mono text-sm font-bold">{order.number}</p><p className="mb-0 mt-2 text-sm text-slate-500">Creada {formatOrderDate(order.createdAt)} · Actualizada {formatOrderDate(order.updatedAt)}</p></div><div className="flex flex-wrap items-center gap-3"><PdfDownloadButton onDownload={() => downloadOrderPdf(useSessionStore.getState().session!.accessToken, order.id)} /><OrderStatus status={order.status} />{canInvoice ? <IconButton className="border-indigo-300 text-indigo-800 hover:bg-indigo-50 focus-visible:ring-indigo-700" icon="file-invoice" label="Facturar orden" onClick={() => setInvoiceOpen(true)} /> : null}{canComplete ? <IconButton className="border-emerald-300 text-emerald-800 hover:bg-emerald-50 focus-visible:ring-emerald-700" icon="check" label="Completar" onClick={() => setCompleteOpen(true)} /> : null}{canCancel ? <IconButton className="border-red-300 text-red-800 hover:bg-red-50 focus-visible:ring-red-700" icon="x" label="Cancelar" onClick={() => setCancelOpen(true)} /> : null}</div></header>
      {canInvoice ? <p className="m-6 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800"><strong>Orden pendiente de facturación.</strong> Usa “Facturar orden” para crear la factura y cambiar el estado de forma atómica.</p> : null}
  ...
  <ConfirmationDialog confirmLabel="Facturar orden" description={`La orden ${order.number} se convertirá en factura y quedará marcada como facturada.`} isPending={mutations.invoice.isPending} onCancel={() => setInvoiceOpen(false)} onConfirm={() => { setNotice(undefined); mutations.invoice.mutate(order.id); }} open={invoiceOpen} title="¿Convertir esta orden en factura?" />
