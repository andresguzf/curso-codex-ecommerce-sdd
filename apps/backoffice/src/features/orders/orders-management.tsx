...
export function OrdersManagement() {
  ...
  const columns: readonly DataTableColumn<OrderRow>[] = [
    ...
    { id: "actions", header: "Acciones", cell: (order) => <div className="flex min-w-40 flex-wrap gap-2"><Link aria-label={`Ver detalle ${order.number}`} className="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-blue-300 text-blue-800 transition hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-700 focus-visible:ring-offset-2" href={`/orders/${order.id}`} title={`Ver detalle ${order.number}`}><Icon name="eye" /></Link>{canManageOrders && order.status === "PROCESSING" ? <IconButton className="border-indigo-300 text-indigo-800 hover:bg-indigo-50 focus-visible:ring-indigo-700" icon="file-invoice" label="Facturar orden" onClick={() => setInvoiceTarget(order)} /> : null}{canManageOrders && order.status === "INVOICED" ? <IconButton className="border-emerald-300 text-emerald-800 hover:bg-emerald-50 focus-visible:ring-emerald-700" icon="check" label="Completar" onClick={() => setCompleteTarget(order)} /> : null}{canManageOrders && (order.status === "PROCESSING" || order.status === "INVOICED") ? <IconButton className="border-red-300 text-red-800 hover:bg-red-50 focus-visible:ring-red-700" icon="x" label="Cancelar" onClick={() => setCancelTarget(order)} /> : null}</div> },
  ];
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-950 sm:px-8"><div className="mx-auto max-w-[90rem]">
    ...
    <div className="flex items-stretch gap-2">
      <section aria-labelledby="orders-list-title" className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 className="m-0 text-lg font-bold" id="orders-list-title">Listado operativo</h2><p className="m-0 text-sm text-slate-600">{query.data ? `${query.data.totalItems} órdenes` : "Consultando órdenes"}</p></div>
      ...
      <div className="flex shrink-0 items-center">
        <IconButton
          className="border-slate-300 text-slate-700 hover:bg-slate-100 focus-visible:ring-blue-700"
      ...
      <CollapsibleSidePanel onClose={() => setFiltersOpen(false)} open={filtersOpen} title="Filtros y orden de órdenes">
        <form className="grid gap-4" key={`filters-${searchParams.toString()}`} onSubmit={(event) => { event.preventDefault(); submitFilters(new FormData(event.currentTarget)); }}>
          ...
          <button className="min-h-11 rounded-lg bg-[#15345b] px-4 font-bold text-white" type="submit">Aplicar filtros</button><Link className="text-center font-bold text-blue-700 underline" href="/orders" onClick={() => setFiltersOpen(false)}>Restablecer</Link>
        </form>
